      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
        <div className="flex items-center gap-2.5">
          ...
          <div className="leading-tight">
            ...
            <p className="-mt-0.5 font-sans text-[11px] uppercase tracking-wide text-paper-400">Học từ vựng mỗi đêm</p>
          </div>
          <SyncIndicator />
        </div>
            ...
            return (
              <button key={item.key} onClick={() => onNavigate(item.key)}
                ...
                {item.key === 'learning' && stats.learning > 0 && (
                  <span className={`ml-0.5 rounded-full px-1.5 py-0.5 text-[11px] font-semibold ${isActive ? 'bg-ink-950/20' : 'bg-ink-700 text-paper-300'}`}>
                    {stats.learning}
                  </span>
                )}
              </button>
